...
**Status:** Implementation complete on 2026-08-01. The event loop merges replay snapshots, stored trades, and deterministic orders; it produces auditable traces, marks, fills, expiry/cancel state, and risk rejections. Fixture tests verify fill behavior. Historical backfill now supplies 373 trades, but a live fill trace remains pending because none overlap a full-depth snapshot ticker/time range.
